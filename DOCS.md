...
- **Routes tab — compact rows + grouping (alpha.16).** Each route renders as a single line by default; click the ▶ chevron on the left to expand the editor inline. The "Group by" dropdown next to **Add route** groups the list (default: by backend target, so e.g. all routes pointing at `10.0.0.20` collapse into one group). Pick a tag from the dropdown to group by it, or **None** for a flat list. Your selection — and which groups you've collapsed — persists in the browser across reloads.
- **Routes tab — tags (alpha.16).** Each route can carry any number of free-form organisational tags (e.g. `proxmox`, `lan-only`, `prod.web`). Edit them via the chip-list in the expanded panel; they surface as small chips on the compact row (max 3 + "+N more"). Tags drive the "Group by" dropdown — defining a tag immediately makes it groupable. Traefik never sees them; pure organisational metadata.
- **Routes tab — system row.** The HA system route is pinned at the top in the "Home Assistant" group. The "System" badge sits in the actions column (right side); the row has no Remove button. Only its hostname (and optionally tags) is editable — backend, scheme, TLS, middlewares, and Skip-TLS are add-on-managed and locked.
